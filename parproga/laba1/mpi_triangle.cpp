...
void init_task(mpi_task_t* task, size_t* pair, float** grid_val, float time) {
    task->time = time;
    task->point[0] = pair[0];
    task->point[1] = pair[1];
    task->phi[0]   = grid_val[pair[0]][pair[1]];
    task->phi[1]   = grid_val[pair[0] - 1][pair[1]];
    task->phi[2]   = grid_val[pair[0]][pair[1] - 1];
}
        ...
        if (rank == 0) {
            ...
            for (size_t layer = 1; layer < pairs->pairs_num; layer ++) {
                for (size_t i = id_left + 1; i < id_right; i ++) {
                    ...
                    mpi_task_t task = {};
                    init_task(&task, pairs[layer].array + 2 * i, equation->phi->grid_val, equation->time);
